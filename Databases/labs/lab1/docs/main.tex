...
%    }
    ...
    \textbf{Стержневые}
    \begin{itemize}
        \item people -- id, name, sex, current\_ship
        \item orbits -- id, type  
        \item spaceships -- id, name, location
        \item celestial\_bodies -- id, type, distance\_description, orbit, landscape, athmosphere
        \item landscapes -- id, appearance\_description
    \end{itemize}

    \textbf{Ассоциативные}  
    \begin{itemize}
        \item actions -- id, type, object, subject
    \end{itemize}

    \textbf{Характеристические}
    \begin{itemize}
        \item shadows -- id, is\_sharp
        \item lights -- id, light\_color, mood
        \item athmospheres -- id, presence, shadow, light
    \end{itemize}
    ...
    \begin{figure}[H] % 'H' -- вставить тут же (подключен модуль), обычный вариант: 'htpb'
        \centering
        \includegraphics[width={\textwidth}]{infological.png}
    ...
    \Subsection{Реализация даталогической модели на SQL}

    \lstinputlisting[label={lst:sql},language=SQL]{../scheme.sql}

    \newpage

    \Section{Вывод}
        При выполнении лабораторной работы я познакомился с основными принципами проектирования баз данных, научился составлять инфологическую и даталогическую модель сущностей, по которым реализовал базу данных с помощью языка `SQL.
    \newpage
