...
\newcommand{\subject}{Веб-программирование}         % учебный предмет
\newcommand{\labtheme}{Разработка HTML-страницы с JS и CSS}            % Тема лабораторной работы
\newcommand{\variant}{№ 12641}                % номер варианта работы
...
%>>>>>>>>>>>>>>>>>>>'''''''''''''''''>>>>>>>>>>>>>>>>>>>>>
   ...
   Разработать HTML-страницу с JS и CSS, определяющую попадание точки на координатной плоскости в заданную область. Область задается по варианту и приведена выше на рисунке. Требуется нарисовать ее средствами Canvas. \\
    ...
    \begin{itemize}
        ...
        \item При работе с CSS должно быть продемонстрировано использование селекторов потомств, селекторов идентификаторов, селекторов элементов, селекторов псевдоэлементов, а также такие свойства стилей CSS, как наследование и каскадирование.
        \item HTML-страница должна иметь «шапку», содержащую ФИО студента, номер группы и номер варианта. При оформлении шапки необходимо явным образом задать шрифт (cursive), его цвет и размер в каскадной таблице стилей.
    ...
    В ходе выполнения задания был создан ряд файлов с расширением .html, .css и .js для верстки, стилизации и логики работы страницы соовественно. \\
    Для удобства отладки и редактирования отдельные модули были вынесены в разные файлы.
    В итоге проект имеет следующую структуру:
    \begin{figure}[H] % 'H' -- вставить тут же (подключен модуль), обычный вариант: 'htpb'
        \centering
        \includegraphics[width=\textwidth/2]{tree.png}
    ...
    Точка входа в программу - файл index.html
    \lstinputlisting[caption={Исходный код главного класса программы},label={lst:java},language=HTMl]{../src/static/index.html}
    ...
    \Section{Результат работы программы.}
        ...
        \caption{Пример таблицы с результатом выполнения}
        \label{fig:diagr}
    \end{figure}
    ...
    \Section{Вывод}
% Вывод...
     В ходе выполнения данной лабораторной работы я изучил основы работы с HTML, CSS и JavaScript. Также я научился создавать формы в HTML, обрабатывать данные из них и возвращать результат на странице. Кроме того, я узнал, как сохранять данные в LocalStorage и получать их оттуда. 
% -- из biblist
    \newpage
%<<<<<<<<<<<<<<<<<<<<<< КОД РАБОТЫ <<<<<<<<<<<<<<<<<<<<<<<<
