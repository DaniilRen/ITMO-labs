...
%    }
    ...
    \Section{Основные этапы вычислений}
		...
		\begin{itemize}
			...
			\item также в нем был создан метод deserialize, осуществляющий десериализацию исходного файла в объект класса Wrapper \ref{lst:formal}. 
    ...
    \Subsection{Дополнительные задание 1}
		...
		Также самописный парсер позволяет обрабатывать вложенность в INI файле (разделение полей точкой), а библиотека configparser этого не делает. \\
    ...
    \Subsection{Дополнительные задание 3}
		...
		\begin{itemize}
			\item для рукописного парсера - затрачено времени 728.8212776184082 мс, 7.288212776184082 мс на 1 итерацию
			\item для парсера с использованием библиотек - затрачено времени 83.90426635742188 мс, 0.8390426635742188 мс на 1 итерацию
		\end{itemize}

		Таким образом, парсера с использованием библиотек выполняет перевод в приблизительно 8-9 раз быстрее. Это может быть связано с тем, библиотеки используют оптимизированные алгоритмы, часто написанные на C/C++ или с использованием специализированных структур данных, что значительно ускоряет обработку. \\
