...
\newcommand{\variant}{№ 46546}                % номер варианта работы
...
%    }
    ...
    \begin{enumerate}
        \item Сделать запрос для получения атрибутов из указанных таблиц, применив фильтры по указанным условиям:\\
            Таблицы: Н{\_}ЛЮДИ, Н{\_}ВЕДОМОСТИ.\\
            Вывести атрибуты:
            \begin{itemize}
                \item Н{\_}ЛЮДИ.ИМЯ,
                \item Н{\_}ВЕДОМОСТИ.ДАТА
            \end{itemize}
            Фильтры (AND):
            \begin{itemize}
                \item Н{\_}ЛЮДИ.ФАМИЛИЯ > Ёлкин.
                \item Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД = 163249.
                \item Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД = 153285.
            \end{itemize}
            Вид соединения: RIGHT JOIN.

        \item Сделать запрос для получения атрибутов из указанных таблиц, применив фильтры по указанным условиям:\\
            Таблицы: Н{\_}ЛЮДИ, Н{\_}ВЕДОМОСТИ, Н{\_}СЕССИЯ.\\
            Вывести атрибуты:
            \begin{itemize}
                \item Н{\_}ЛЮДИ.ИМЯ, 
                \item Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД,
                \item Н{\_}СЕССИЯ.ЧЛВК{\_}ИД.
            \end{itemize}
            Фильтры (AND):
            \begin{itemize}
                \item Н{\_}ЛЮДИ.ФАМИЛИЯ > Иванов
                \item Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД > 142390.
            \end{itemize}
            Вид соединения: RIGHT JOIN.
    ...
    \subsection{Реализация запросов на SQL}

    \lstinputlisting[label={lst:sql},language=SQL]{../script.sql}

    \subsection{Индексы}

    \textbf{Запрос 1}

    так как в запросе с самого начала присутсвуют противоречащие фильтры (Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД = 163249 и Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД = 153285),
    планировщие запросов сразу распознает это и выдает 0 строк. Чтобы добавление индексов имело смысл, изменим условия данного запроса на: 
    \begin{itemize}
            \item Н{\_}ЛЮДИ.ФАМИЛИЯ > Ёлкин.
            \item Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД < 163249.
            \item Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД > 153285.
    ...
    \begin{enumerate}
        \item B-tree индекс на атрибуте Н{\_}ЛЮДИ.ФАМИЛИЯ. Используем B-tree, так как нас интересует диапазон значений.
         Это ускорит обработку фильтра Н{\_}ЛЮДИ.ФАМИЛИЯ > Ёлкин.
        \item B-tree индекс Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД. Используем B-tree, так как нас интересует диапазон значений.
         Это ускорит выполнение фильтров 
            Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД < 163249 и Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД > 153285, 
            а также соединение таблиц по условию ON Н{\_}ЛЮДИ.ИД = Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД.
    \end{enumerate}
    ...
    \begin{enumerate}
        \item B-tree индекс на атрибуте Н{\_}ЛЮДИ.ФАМИЛИЯ. Используем B-tree, так как нас интересует диапазон значений.
         Это ускорит обработку фильтра Н{\_}ЛЮДИ.ФАМИЛИЯ > Иванов.
        \item Hash индекс Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД. Используем Hash, так как нас интересует конкретное значение.
         Это ускорит выполнение фильтра Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД = 142390, а также соединение таблиц по условиям ON Н{\_}ЛЮДИ.ИД = Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД, 
         ON Н{\_}СЕССИЯ.ЧЛВК{\_}ИД = Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД.
         \item B-tree индекс Н{\_}СЕССИЯ.ЧЛВК{\_}ИД.
         Это ускорит соединение таблиц по условию ON Н{\_}СЕССИЯ.ЧЛВК{\_}ИД = Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД.
    \end{enumerate}
    ...
    Для обоих запросов второй план является более оптимальным, так как выборка происходит на более ранних этапах и, как следствие, в объединении отношений участвует меньшее количество строк.
    Добавление индексов не повлияет на план выполнения запроса, а только ускорит производительность.

    \subsection{Вывод EXPLAIN-ANALYZE}

    \lstinputlisting[label={lst:sql},language=SQL]{../analyze_output.sql}
