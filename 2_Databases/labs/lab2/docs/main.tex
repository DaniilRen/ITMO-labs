...
\newcommand{\variant}{№ 6671}                % номер варианта работы
...
%    }
    ...
    \begin{enumerate}
        \item Сделать запрос для получения атрибутов из указанных таблиц, применив фильтры по указанным условиям: \newline
            Таблицы: Н{\_}ОЦЕНКИ, Н{\_}ВЕДОМОСТИ.  \newline
            Вывести атрибуты:
            \begin{itemize}
                \item Н{\_}ОЦЕНКИ.ПРИМЕЧАНИЕ,   
                \item Н{\_}ВЕДОМОСТИ.ЧЛВК{\_}ИД.
            \end{itemize}  
            Фильтры (AND):  \newline
            a) Н{\_}ОЦЕНКИ.КОД < 4.  \newline
            b) Н{\_}ВЕДОМОСТИ.ИД < 39921.  \newline
            c) Н{\_}ВЕДОМОСТИ.ИД < 1490007.  \newline
            Вид соединения: LEFT JOIN.  \newline

        \item Сделать запрос для получения атрибутов из указанных таблиц, применив фильтры по указанным условиям: \newline
            Таблицы: Н{\_}ЛЮДИ, Н{\_}ОБУЧЕНИЯ, Н{\_}УЧЕНИКИ. \newline
            Вывести атрибуты: 
            \begin{itemize}
                \item Н{\_}ЛЮДИ.ОТЧЕСТВО, 
                \item Н{\_}ОБУЧЕНИЯ.НЗК, 
                \item Н{\_}УЧЕНИКИ.НАЧАЛО.
            \end{itemize}
            Фильтры: (AND) \newline
            a) Н{\_}ЛЮДИ.ФАМИЛИЯ = Петров. \newline
            b) Н{\_}ОБУЧЕНИЯ.ЧЛВК{\_}ИД < 113409. \newline
            Вид соединения: INNER JOIN. \newline

        \item Вывести число дней без учета повторений. \newline
            При составлении запроса нельзя использовать DISTINCT.

        \item Выдать различные имена студентов и число людей с каждой из этих имен, ограничив список именами, встречающимися менее 50 раз на на заочной форме обучения. \newline
            Для реализации использовать подзапрос.
        \item Выведите таблицу со средним возрастом студентов во всех группах (Группа, Средний возраст), где средний возраст равен минимальному возрасту в группе 1100.
        \item Получить список студентов, отчисленных до первого сентября 2012 года с заочной формы обучения (специальность: Программная инженерия). В результат включить:
        \begin{itemize}
            \item номер группы;
        ...
        \item Вывести список людей, не являющихся или не являвшихся студентами ФКТИУ (данные, о которых отсутствуют в таблице Н{\_}УЧЕНИКИ). В запросе нельзя использовать DISTINCT.
    ...
    \Section{Вывод}
        При выполнении лабораторной работы я познакомился с основными функциями SQL и PostgreSQL, научился писать различные запросы: получать, агрегировать, фильтровать и сортировать данные. Мной был освоен язык DML SQL, используемый для работы с данными, хранящимися в базе.
    \newpage
