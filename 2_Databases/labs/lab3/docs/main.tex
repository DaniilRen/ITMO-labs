...
\newcommand{\variant}{№ 501993}                % номер варианта работы
...
%    }
    ...
    \Subsection{Исходная даталогическая модель}

    \noindent
    \begin{figure}[H] 
        \centering
        \includegraphics[width={\textwidth}]{datalogical.png}
    ...
    \begin{enumerate}
        \item id $\to$ name
        \item id $\to$ sex 
        \item id $\to$ $current\_ship$ 
    \end{enumerate}
    
    Actions
    \begin{enumerate}
        \item (type, object, subject) $\to$ type
        \item (type, object, subject) $\to$ object 
        \item (type, object, subject) $\to$ subject 
    \end{enumerate}

    Celectial bodies
    \begin{enumerate}
        \item id $\to$ type
        \item id $\to$ $distance\_description$ 
        \item id $\to$ orbit
        \item id $\to$ landscape
        \item id $\to$ athmosphere
    \end{enumerate}
    ...
    Landscapes
    \begin{enumerate}
        \item id $\to$ $appearence\_description$ 
    \end{enumerate}
    ...
    \Subsection{Отношения в НФ}
    \textbf{Преобразование к 1НФ}\\
    ...
    \textbf{Преобразование к 3НФ}\\
    Отношение находится в 3НФ, если оно уже находится в 2НФ и все атрибуты, которе не входят в первичный ключ, не находятся в транзитивной функциональной зависимости от первичого ключа. Для моей модели это уже выполняется

    \textbf{Преобразование к НФБК}\\
    Отношение находится в НФБК, если оно уже находится в 3НФ и для всех функиональных зависимостей отношения выолняется условие: детерминант - потенциальный ключ. \\
    ...
    \textbf{Преобразование к 6НФ}\\
    Отношение находится в 5НФ, если оно имеет только 1 первичный ключ и один дополнительный атрибут. Моя модель не соовествует данной форме, поэтому для примера можно разбить отношение Celestial bodies по 6НФ:
    \noindent
    \begin{figure}[H] 
        \centering
        \includegraphics[width={\textwidth}]{6NF.png}
        \caption{Celestial bodies 6NF}
        \label{fig:enter-label}
    \end{figure}
    ...
    \lstinputlisting[label={lst:sql},language=SQL]{../trigger.sql}

    \newpage

    \Section{Вывод}
        При выполнении лабораторной работы я познакомился с методами нормализации и денормализации баз данных, научился создавать тригеры и функции в PL/pgSQL.
    \newpage
