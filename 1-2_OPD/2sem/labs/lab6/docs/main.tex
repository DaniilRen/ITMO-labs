...
%    }
    ...
    \begin{enumerate}
        \item Основная программа должна уменьшать на 3 содержимое X (ячейки памяти с адресом 04F) в цикле. 
...
\lstinputlisting[label={lst:asm},language=]{../asm.bcomp}
...
\Subsection{Проверка}
\textbf{Проверка основной программы}
\begin{enumerate}
    \item загрузить код в БЭВМ
    \item изменить значение 1 точки отладки на HLT
    \item запустить программу с адреса 0x053
    \item дождаться остановки и запомнить адрес IP
    \item прочитать и запомнить значение переменной X по адресу 0x04F
    \item продолжить выполнение программы с предыдущего адреса IP
    \item дождаться остановки и сравнить значение X с предыдущим. Оно должно быть либо меньше на 6 (так как вычелось 2 раза), лиюо равно максимальному числу в ОДЗ (0x14)
\end{enumerate}

Пример:\\
При изначальном X=0x6 мы получили IP=06B, 5
