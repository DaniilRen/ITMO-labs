...
\newcommand{\labnumber}{№ 8}                    % порядковый номер работы
\newcommand{\subject}{Программирование}         % учебный предмет         % Тема лабораторной работы
\newcommand{\variant}{№ 89493}                % номер варианта работы
...
%>>>>>>>>>>>>>>>>>>>'''''''''''''''''>>>>>>>>>>>>>>>>>>>>>
    ...
    \textbf{Доработать программу из лабораторной работы №7 следующим образом:}\\

    Графический интерфейс клиентской части должен поддерживать русский, македонский, французкий и английский (Соединенные Штаты) языки / локали. Должно обеспечиваться корректное отображение чисел, даты и времени в соответстии с локалью. Переключение языков должно происходить без перезапуска приложения. Локализованные ресурсы должны храниться в классе.
    ...
    \begin{enumerate}
        ...
        \item Таблица, отображающая все объекты из коллекции
        \begin{itemize}
        ...
        \item Область, визуализирующую объекты коллекции
        \begin{itemize}
        ...
        \end{itemize}
        \item Возможность редактирования отдельных полей любого из объектов (принадлежащего пользователю). Переход к редактированию объекта возможен из таблицы с общим списком объектов и из области с визуализацией объекта3
    ...
    \Section{Выполнение задания.}
        При разработке использовался JavaFX. 
		Задание было выполнено в редакторе кода Visual Studio Code, затем при помощи системы сборки Maven собрано в несколько \verb|jar| архивов:
		\begin{enumerate}
			\item \verb|server-1.0-SNAPSHOT.jar|
			\item \verb|client-1.0-SNAPSHOT.jar|
			\item \verb|common-1.0-SNAPSHOT.jar|
		\end{enumerate}
		Исходный код программы был загружен в Git репозиторий на GitHub \cite{github}.
    \Subsection{Исходный код программы}
    
    \url{https://github.com/DaniilRen/ITMO-labs/tree/main/1-2_Java/2sem/labs/Lab8}

    \Subsection{UML-диаграмма классов}
    
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{Lab6.png}
        \caption{UML диаграмма}
        \label{fig:diagr}
    \end{figure}

    Ссылка на диаграмму: \url{https://github.com/DaniilRen/ITMO-labs/tree/main/1-2_Java/2sem/labs/Lab8/docs/assets/Lab8.png}

    % \newpage

    \Subsection{Архитектура клиента}

    При разработке клиента был использован паттерн MVP (Model-View-Presenter). 
    \begin{itemize}
        \item Взаимодействие с сетью, сложные операции исполнения и обработки, универсальные для любого клиента были вынесены в Model
        \item View представляет собой внешний интерфейс с по большей части пассивной моделью взаимодействия с пользователем - перехватывает события и отдает на обработку Model
        \item Presenter является мостиком между первыми 2 классами, абстрагируя конкретную реализацию модели от представления
    \end{itemize}
